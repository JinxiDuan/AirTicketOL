...
bool BookUserExited(string username, int &userposi) {
    ...
    for (int i = 0; i < allbooknum; i++) {//逐个检验
...
void book(int thisposi, string username) //订票菜单
{
    BookInfo bookhere;
    int NumOfTicbked = 0;
    int bookn = 0;
    ...
    cout << "请输入订票张数：";
    cin >> NumOfTicbked;
    if (fli.ReturnSeats() >= NumOfTicbked) //有余票
    {

        int ExitedBookposi;//存在的用户预定位置
        if (BookUserExited(username, ExitedBookposi)) {//用户之前预定过票
            BookInfo bkif2;
            int TPFposi;
            ...
            if (BookNoExited(bkif2, fli.ReturnFliNO(), TPFposi)) {//用户之前预定过该趟航班
...
void book(int *thisposi, int n, string username) //多种票类订票菜单
{
    Flight flirsrved;
    BookInfo bookinfo;
    int BookNo;
    ...
    if (flirsrved.ReturnSeats() >= NumOfTicbked) //有余票
    {
        fstream bookinfo("bookinfo.dat", ios::in | ios::out | ios::binary);
        if (BookUserExited(username, ExitedBookposi)) //用户使用过预定系统
        {
            BookInfo bkif2;
            int TPFposi;
            bookinfo.seekg((sizeof(int)) + (ExitedBookposi * sizeof(BookInfo)), ios::beg);
                            ...
                            lb6:
                                ...
                                switch (j) {
                                    case '1': {
                                        cout << "\n请输入航班号：";
                                        cin >> TypeFliNo;
                                        if (SearchFliNo(TypeFliNo, soposi)) //可以查找到相关信息
                                        {
                                            cout << "\n查找到以下航班信息：\n\n";
                                            //显示相关航班
                                    ...
                                    case '2': {
                                        ...
                                        cout << "\n请输入降落城市：";
                                        cin >> TypeArrivC;
                                        if (SearchFliC(TypeDepartC, TypeArrivC, &theseposi, relatedFlin)) //可以查找到相关信息
                                        {
                                            cout << "\n查找到以下航班信息：\n\n";
                                    ...
                                    case '3': {
                                        ...
                                        if (NoorCity.c_str()[0] < 0) {
                                            ...
                                            if (SearchFliNo(NoorCity, FliRn)) //可以查找到相关信息
                                            {
                                                cout << "\n查找到以下航班信息：\n\n";
                                                //显示相关航班
                                                ShowthisFli(FliRn, 0);
